...
data <- data %>% dplyr::mutate(OR = dplyr::case_when(OR < 1 ~ -1/OR, OR >= 1 ~ OR)) %>% dplyr::mutate(signif = dplyr::case_when(fdr <= 0.05 ~ 1, fdr > 0.05 ~ 0))
...
if (type == "CGI") {
p <- p + scale_fill_manual(values = cpgPal)
}
else if (type == "Genic") {
